...
# Note:   Parse the detailed logs for LlamaCpp and MLCChat++ and compute the power consumption and performance metrics.
...
def compute_power_performance(df, current_col="current", voltage_col="voltage"):
    ...
    df = df[["timestamp", current_col, voltage_col]].copy()
    df[voltage_col] = df[voltage_col].bfill()
...
def load_energy_events(filepath):
    ...
    df = pd.read_csv(
        filepath, header=None, delimiter=" ", index_col=0, names=["event", "value"]
    )
    return df
...
def check_app_type(filepath):
    ...
    with open(filepath, encoding="utf-8", errors="ignore") as file:
        ...
        if "main" in first_line:
            ...
            sys.exit(
                f"Error: Could not infer application type from '{filepath}'.")


def compute_llamacpp_performance_metrics(
    filepath_csv, filepath_txt, iteration, conversation, mdf
):
    ...
    for start_time, row in df.iterrows():
        ...
        # power consumption
        df_trimmed = mdf[(mdf.timestamp > start_time)
                         & (mdf.timestamp < end_time)]
        ...
        relevant_power_events = [
            column.replace("current_", "").replace(" (mA)", "")
        ...
        if row.state == "load_model":
            ...
            for power_event in relevant_power_events:
                total_energy, total_discharge = compute_power_performance(
                    df_trimmed,
                    f"current_{power_event} (mA)",
                    f"voltage_{power_event} (V)",
                )
            ...
            for power_event in relevant_power_events:
                ...
                entry[f"energy_pt {power_event} (mWh)"] = total_energy / \
                    output_tokens
                entry[f"discharge_pt {power_event} (mAh)"] = (
                    total_discharge / output_tokens
                )
...
def compute_mlc_performance_metrics(
    filepath_csv, filepath_txt, iteration, conversation, mdf
):
    ...
    with open(filepath_txt, encoding="utf-8") as f:
        txt_lines = f.readlines()
    txt_lines = [ansi_escape.sub("", line) for line in txt_lines]
    # remove lines containing WARNING: your terminal doesn't support cursor position requests
    txt_lines = [
        line
        for line in txt_lines
        if "WARNING: your terminal doesn't support cursor position requests" not in line
    ]
    # Remove empty lines
    ...
    for idx in json_idxs:
        try:
            json_str = "".join(
                txt_lines[idx: idx + TOTAL_STATS_LINES_PER_PROMPT])
            # json_str = json_str[json_str.find("{"):]
            stats = json.loads(json_str)
            mlc_stats.append(stats)
        except JSONDecodeError:
            print(
                "Skipping run because of failed stats collection. (expected once per run for new MLCChat++)"
            )
            continue
    ...
    for start_time, row in df.iterrows():
        ...
        if row.state == "load_model":
            ...
            # get relevant metrics from txt file
            try:
                stats = mlc_stats[prompt_idx]
            except IndexError:
                print("Skipping run because of failed stats collection.")
                continue
            ...
            try:
                input_tokens = int(
                    stats["prefill"]["total tokens"].replace("tok", ""))
                output_tokens = int(
                    stats["decode"]["total tokens"].replace("tok", ""))
            except KeyError:
                input_tokens = int(
                    stats["prefill"]["total_tokens"].replace("tok", ""))
                output_tokens = int(
                    stats["decode"]["total_tokens"].replace("tok", ""))
            ...
            for power_event in relevant_power_events:
                df_trimmed_current_events = df_trimmed[
                    df_trimmed["event"] == f"current_{power_event} (mA)"
                ]
                total_energy, total_discharge = compute_power_performance(
                    df_trimmed_current_events,
                    f"current_{power_event} (mA)",
                    f"voltage_{power_event} (V)",
                )
                entry[f"energy_pt {power_event} (mWh)"] = total_energy / \
...
def compute_detailed_performance_metrics(
    ...
    for index, row in df_start.iterrows():
        ...
        if len(end_event) != 1:
            print(
                f"WARNING: Expecting one matching end_event for '{stage}' and discovered {len(end_event)}. Skipping..."
            )
            continue
        else:
    ...
    for filepath_log in sorted(glob.glob(os.path.join(path, "energy_iter[0-9]*.log"))):
        ...
        for filepath_csv in sorted(
            ...
            if check_app_type(filepath_txt) == "llamacpp":
                model_perf_metrics, inf_perf_metrics = (
                    compute_llamacpp_performance_metrics(
                        filepath_csv, filepath_txt, iteration, conversation, energy
                    )
                )

            elif check_app_type(filepath_txt) == "mlc":
                model_perf_metrics, inf_perf_metrics = compute_mlc_performance_metrics(
                    filepath_csv, filepath_txt, iteration, conversation, energy
                )
            ...
            try:
                ...
                detailed_metrics_list.append(
                    compute_detailed_performance_metrics(
                        filepath_tsv, iteration, conversation, energy, csv_sep=" "
                    )
                )
